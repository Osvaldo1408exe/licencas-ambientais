...
public class LicencaDAO {
    ...
    //Busca apenas o item selecionado

    public Optional<Licenca> findById(int id) {

        String sql = """
            SELECT licencas.id_licenca, area.descricao as area, unidade.descricao as unidade, sub_unidade.descricao as subUnidade, licencas.data_requerimento,
                    ...
                    numero_licenca, fcei_sinfat, num_processo_sinfat, sgpe, processo_sei, data_emissao, data_vencimento, previsao.descricao as previsao,
                    sim_nao.descricao as requerimento, data_protocolo_orgao, sim_nao2.descricao as emitida_nova_licenca, situacao_processo.descricao as situacao_processo,
                    sim_nao.descricao as atualizad_sa, observacoes, providenciar_doc, data_limite, tempo_tramitacao, situacao_licenca.descricao as situacao_licenca,
                    setor_responsavel.descricao as setor_responsavel, dias_para_vencer, licencas.ativo
            FROM licencas
            JOIN area ON licencas.id_area = area.id_area
        ...
        try (Connection conn = ConnectionFactory.getConnection();
            ...
            if (rs.next()) {
                Licenca licenca = new Licenca(
                        rs.getInt("id_licenca"),
                        rs.getString("area"),
                        rs.getString("unidade"),
                        rs.getString("subUnidade"),
                        rs.getDate("data_requerimento"),
                        rs.getString("controle"),
                        rs.getString("orgao_emissor"),
                        rs.getString("tipo"),
                        rs.getString("especificacao"),
                        rs.getString("numero_licenca"),
                        rs.getInt("fcei_sinfat"),
                        rs.getString("num_processo_sinfat"),
                        rs.getString("sgpe"),
                        rs.getString("processo_sei"),
                        rs.getDate("data_emissao"),
                        rs.getDate("data_vencimento"),
                        rs.getString("previsao"),
                        rs.getString("requerimento"),
                        rs.getDate("data_protocolo_orgao"),
                        rs.getString("emitida_nova_licenca"),
                        rs.getString("situacao_processo"),
                        rs.getString("atualizad_sa"),
                        rs.getString("observacoes"),
                        rs.getDate("providenciar_doc"),
                        rs.getDate("data_limite"),
                        rs.getInt("tempo_tramitacao"),
                        rs.getString("situacao_licenca"),
                        rs.getString("setor_responsavel"),
                        rs.getInt("dias_para_vencer"),
                        rs.getString("ativo")
                );
                return Optional.of(licenca); // Retorna um Optional com o objeto Licenca
            }

        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
        return Optional.empty(); // Retorna um Optional vazio se não encontrar a licença
    }
